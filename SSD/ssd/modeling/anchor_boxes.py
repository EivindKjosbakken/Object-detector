    ...
    def __init__(self, 
        ...
        for fidx, [fH, fW] in enumerate(feature_sizes):
            ...
            for r in aspect_ratios[fidx]:
                #TODO endrer her -> hvis pixel er under halvveis -> legge til flere boxes

                h = h_min*sqrt(r)
                w = w_min/sqrt(r)
                bbox_sizes.append((w_min*sqrt(r), h_min/sqrt(r))) #TODO fixed bug here
            ...
            for w, h in bbox_sizes:
                for i in range(fH):
                    for j in range(fW):
                        cx = (j + 0.5)/scale_x
                        cy = (i + 0.5)/scale_y #TODO her er center
