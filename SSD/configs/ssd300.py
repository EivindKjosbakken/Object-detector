...
#to easiliy be able to tune this
number_of_workers = 0
...
backbone = L(backbones.BasicModel)(
    output_channels=[64, 128, 128, 128, 64, 64],
...
data_train = dict(
    ...
    dataloader=L(torch.utils.data.DataLoader)(
        dataset="${..dataset}", num_workers=number_of_workers, pin_memory=True, shuffle=True, batch_size="${...train.batch_size}", collate_fn=utils.batch_collate,
        drop_last=True
    ),
...
data_val = dict(
    ...
    dataloader=L(torch.utils.data.DataLoader)(
        dataset="${..dataset}", num_workers=number_of_workers, pin_memory=True, shuffle=False, batch_size="${...train.batch_size}", collate_fn=utils.batch_collate_val
    ),
    gpu_transform=L(torchvision.transforms.Compose)(transforms=[
