...
from mAPHolder import mAPHolder #TODO implementing early stop and saving of best checkpoint
...
def train(config_path: Path, evaluate_only: bool):
    ...
    earlyStopCount = 12
    mAPHolderObject = mAPHolder(earlyStopCount) 
    ...
    for epoch in range(start_epoch, cfg.train.epochs):
        ...
        #TODO adding early stop #TODO funker denne nå?
        if "metrics/mAP" in eval_stats.keys():
            print("MAP NOW WAS: ", eval_stats["metrics/mAP"])
            mAPHolderObject.addmAP(float(eval_stats["metrics/mAP"]))
            if mAPHolderObject.checkIfShouldEarlyStop(float(eval_stats["metrics/mAP"])):
        ...
        #to make sure to save best model:
        if "metrics/mAP" in eval_stats.keys():
            epochmAP = eval_stats["metrics/mAP"] #the mAP for this epoch
            if epochmAP > mAPHolder.bestmAP: #have a class attribute called bestmAP (class attribute, and not object attribute, so I can easly access it from any class)
                checkpointer.save_registered_models(train_state, is_best = True)
                mAPHolder.bestmAP = epochmAP #set new best mAP
                print("\n_______________________\n")
                print("\n ___BEST MAP YET___ \n")
                print("\n_______________________\n")
            else:
                checkpointer.save_registered_models(train_state, is_best = False)
        else:
            checkpointer.save_registered_models(train_state, is_best = False)
